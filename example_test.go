...
func Example() {
	...
	resp, err := request.NewBuilder().
		Timeout(time.Second*10). // Override whatever is set on the client.
		BasicAuth("username", "password").
		JSONBody(&payload{"This is an example."}).
		DoWithContext(ctx, http.MethodPost, "http://localhost")
	if err != nil {
		panic(err)
	}
